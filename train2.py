...
for epoch in range(epochs):
    ...
    for i in range(len(batches)):
        ...
        for j in range(cur_batch):
            seq.append( ft[j].shape[0] )
            batch.append( torch.Tensor(ft[j]) )
    ...
    if epoch == epochs-1:
        torch.save(net.state_dict(), '{}model-rnn.pth'.format(path_checkpoint))
